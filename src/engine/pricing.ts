...
export function goodById(goodId: string): Good | undefined {
  if (GOODS_BY_ID[goodId]) return GOODS_BY_ID[goodId];
  const m = sectorGoodRe.exec(goodId);
  if (!m) return undefined;
  const sector = parseInt(m[1], 10);
  return generateSectorGoods(sector).find((g) => g.id === goodId);
}

export function biasFor(stationId: string, good: Good): number {
  ...
  const table = generateSectorBias(allStationIds(), generateSectorGoods(sector), sector);
  return table[stationId]?.[good.id] ?? 1;
}
...
export function getPrice(state: GameState, stationId: string, goodId: string): number {
  ...
  const bias = biasFor(stationId, good);
...
export function goodsCatalogForState(state: GameState): Good[] {
  ...
  for (let s = 2; s <= state.maxSectorReached; s++) list.push(...generateSectorGoods(s));
  return list;
}
